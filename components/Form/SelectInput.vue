      ...
      <div class="input-control" v-click-outside="closeDropdown" @click="openDropdown">
        ...
        <span class="inline-block w-full rounded-md shadow-sm">
          <button type="button" aria-haspopup="listbox" aria-expanded="true" aria-labelledby="listbox-label" class="relative w-full rounded-md border border-gray-300 bg-white pr-10 text-left focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition ease-in-out duration-150 sm:text-sm sm:leading-5 inputi">
            ...
            <span class="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none cursor-pointer">
...
.input-control {
  ...
  label {
    @apply transform -translate-y-1/2 absolute left-4 transition-all text-gray-400 top-1/4 text-sm z-10 cursor-pointer;
  }
  .inputi {
    @apply w-full h-12 pl-4 pt-4 text-xl font-normal cursor-pointer;
  }
}
</style>
